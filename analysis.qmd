---
title: "DATA589 Spatial Project - Pine White Butterfly in BC"
...
M <- persp(DATA$Elevation,
           ...
           cex.axis = 1.2,
           cex.main = 1.6,
           main = "Neophasia menapia Distribution over Elevation")
...
legend("topright", legend = "Butterfly Observations",
       pch = 21, pt.bg = "orange", col = "black", bty = "n")
...
```{r}
lambda_est <- density(data_ppp_clean)
k_inhom <- Kinhom(data_ppp_clean, lambda=lambda_est)
plot(k_inhom)
```
...
```{r}
fit_best <- ppm(data_ppp_clean_unmark ~ elev + water + forest + I(forest^2) + HFI + I(HFI^2))
